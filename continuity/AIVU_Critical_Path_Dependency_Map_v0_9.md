...
**Version:** 0.9 — finalized 2026-05-21 at session close per the End-of-Session checklist. Supersedes v0.8 (2026-05-19 end of session), which captured two sessions of HVAC-track work — the 2026-05-18 commissioning-track bootstrap and the 2026-05-19 Day 3-4 protocol spec. v0.9 adds T11 (end-to-end Day2→Day4→Day6 chain integration test), T12 (greybox environment portability, completed 2026-05-20), and T13 (analytic-truth forward verification battery, committed 2026-05-21); records the completion of Architectural Distillation v0.2; and folds in the 2026-05-21 debugging session, which found and fixed three forward-chain bugs — C_house optimizer conditioning, the C_w §11.2 specification defect, and frozen-SHR over-drying — bringing §6 closed-loop to 3/3 passing against measured HVAC. The v0.4 architectural insights stand. The reformulation continues to validate against real physics across all stages reached.
...
**Insight 1: Sequential identification by frequency band within Cx.** Joint fit could not identify seven parameters from §5 passive observation (Hessian condition number 2.9 × 10^14). Structural fix: four stages, frequency-bands separated, posterior-as-prior propagation. Validated 2026-05-17 against real Phase 1 + aivu_dynamic at 24h and 48h Phoenix-July windows. Stage 3 + Stage 4 now also validated against measured HVAC — 2-of-3 in Step 2 (2026-05-18), reaching 3-of-3 on 2026-05-21 after the C_house, C_w, and SHR forward-chain bugs were diagnosed and fixed.
...
1. **Envelope half.** End-of-Day-6 `Day6Posterior` record with `AttestationMoment.ENVELOPE_HALF_FINAL`. Five parameters at `confidence_state = "production-threshold-at-Cx"` (R_opaque, U_fenestration, ceiling_coupling_factor at Stage 1; C_house at Stage 3; C_w at Stage 4); two parameters at `confidence_state = "preliminary-refining-at-Cx"` (C_stack, C_wind at Stage 2 — these two are by design *not* expected to reach production tightness within Cx; per Insight 2 their primary identification horizon is operational telemetry, so a closed-loop test that leaves C_stack/C_wind at preliminary confidence is behaving as designed, not failing). Signed by the real `aivu_integrity` chain.
...
### T13 (NEW, 2026-05-21). Analytic-truth forward verification battery
- State: `NOT_STARTED`; committed as a workstream 2026-05-21. **Unblocked** — the live-evaluation HVAC provider it sequences after is complete (see Open Questions, the SHR resolution).
- Extends the existing D1–D5 discipline. Motivation: closed-loop recovery tests share the forward chain between truth and fit, so a defect *in the forward physics* passes green on both sides — the structural blind spot that let all three 2026-05-21 bugs (C_house optimizer conditioning, C_w §11.2 spec defect, frozen-SHR over-drying) through undetected across multiple sessions. The battery verifies the forward model against hand-computable answers on deliberately trivial inputs, independent of the chain itself.
- Four cases, each isolating one channel: a **sensible-only** case (cube, uniform U, constant T_out, no sun, no HVAC → closed-form steady-state ΔT and τ = R·C); a **latent-only** case (fixed moisture source + dehumidification → closed-form moisture balance, W_in stays positive — the channel that broke on 2026-05-21); a **coupled HVAC-on** case (constant outdoor condition → delivered capacity, SHR split, and steady-state (T,W) all matching a hand calculation — exercises the live-evaluation architecture); and a **conservation check** (energy/moisture in − out − stored = 0 to machine precision). Cheap to run.
- **Complementary to T11's weather-file sweep, not redundant with it.** The weather sweep runs truth and fit through the *same* forward chain across many EPWs: it probes the *fit's robustness* to excitation and operating-region variation, but cannot catch a forward-physics defect, which is present identically on both sides regardless of weather. The analytic battery checks *forward-model correctness* against answers that do not come from the chain. Both are needed; they catch different failure classes, and neither substitutes for the other.
- **Sequencing:** built against the completed, corrected forward model — the live-evaluation provider it depends on is now finished — then guards every change after that. Effort: ~1 session. Owner decision (JDS, 2026-05-21).
...
**Pilot-blocking software sequence — UPDATED in v0.9:**

1. **Analytic-truth forward verification battery (T13).** Built against the now-corrected forward model; the independent trust foundation for every closed-loop result after it. Unblocked as of 2026-05-21. ~1 session.
...
1. **C_house bias — RESOLVED 2026-05-21.** The Stage 3 C_house over-recovery (4.893e6 vs truth 4.5e6, ~3.9σ), carried as an open question since 2026-05-18, is diagnosed and fixed. The 2026-05-20 "live finding" — that an unphysically low `moisture_removal_kg_per_s` default was the dominant residual — was itself a plausible-but-wrong hypothesis. It was falsified along with four others (eta-distribution handling, the SHR/total-vs-sensible channel, prior pull, a biased Welch objective, co-parameter absorption), each killed by a short probe. The real cause: optimizer coordinate conditioning in `_laplace_with_objective` (`staged_fit.py`). The seven fitted parameters span ~7 orders of magnitude; L-BFGS-B applies a single step-length across all coordinates, which effectively froze C_house near its starting value. Fix: the optimizer now runs in normalized coordinates (u = θ / scale, scale = the prior's marginal sigmas), with bounds and restart starts mapped into u-space and `result.x` mapped back. Verified — Stage 3 C_house recovers to truth; §6 closed-loop passes 3/3. Committed 2026-05-21. Retained here one version; drops at v1.0.
2. **C_w recovery — RESOLVED 2026-05-21.** Exposed once the C_house fix unfroze the Stage 4 fit: C_w recovered far from truth under the then-current units. Root cause: a specification defect in the §11.2 amendment — C_w was named as the whole-house lumped latent moisture capacitance but routed to `run_dynamic`'s `kappa_buffer_override` (the κ_buffer *coefficient* slot) with no conversion, unlike the C_house entry, which specifies its conversion. Decision (JDS, Option B): C_w *is* the whole-house capacitance, SI units (kg); `real_chain` converts to the κ_buffer coefficient by inverting `capacitance.py`'s affine map, probing the public moisture-capacity API at two κ values. Fix applied across four files (`real_chain.py` conversion, the `staged_fit.py` floor, the `passive_fit_types.py` fallback prior, the closed-loop test's perturbed truth), and the §11.2 amendment corrected to specify the conversion parallel to the C_house entry. Prior re-derived (mean 2400 kg, σ 1000 kg; truth 2100 kg). Verified — Stage 4 C_w recovers; §6 closed-loop passes 3/3. Committed 2026-05-21. Retained here one version; drops at v1.0.
3. **T9 §6 staged closed-loop against real chain.** v0.7 priority carried; <1 session.
...
6. **Day 3-4 protocol spec lock criterion.** v0.1 ships as DRAFT_SHIPPED. v0.2 spec-lock per the spec's closing note requires pilot Day 3-4 execution producing a Day4Posterior that H2 consumes successfully and that the §6 envelope active commissioning (Days 5-6) consumes successfully. Until then, v0.1 is the working document and sweep_orchestrator code is drafted against it.
7. **H2 sensible/latent decomposition for the real-chain path — reconciled 2026-05-21.** H1 v0.2 §3.4 deferred the coil sensible/latent split; H2 fits total delivered capacity only. The 2026-05-21 SHR fix resolved *where the §6 sensible drive comes from*: the §6 synthesizer no longer hand-sets moisture removal — it derives the sensible/latent split from the D19 coil-SHR characterization, evaluated live against indoor wet-bulb, and the fit consumes the resulting recorded trace. So the answer to "the real pilot HPM measures total delivered capacity; where does the sensible split come from" is now concrete: from the coil-SHR characterization (D19). The remaining open part is narrower than before: D19 is presently a placeholder (`PLACEHOLDER_D19_SHR`), and §6 now structurally depends on it. D19 must graduate from placeholder to a fitted/anchored characterization before the real-chain H2 path; whether H1 §3.4's deferral of the split remains correct, given §6's demonstrated structural dependence on D19, is the decision to settle.

---

*End of v0.9, finalized 2026-05-21. Adds T11 (end-to-end Day2→Day4→Day6 chain integration test), T12 (greybox environment portability, completed 2026-05-20), and T13 (analytic-truth forward verification battery, committed 2026-05-21); records Architectural Distillation v0.2 as complete; and folds in the 2026-05-21 debugging session, which diagnosed and fixed three forward-chain bugs — C_house optimizer conditioning, the C_w §11.2 specification defect, and frozen-SHR over-drying — bringing §6 closed-loop to 3/3 passing against measured HVAC. The C_house and C_w resolutions are retained one version in Open Questions and drop at v1.0. v0.4's architectural insights stand; the reformulation continues to validate against real physics across all stages reached. The next session opens against this finalized v0.9; if anything was relitigated mid-session and resolved differently, this file is stale and v1.0 reconciles before reasoning forward.*
